...
export default function Home() {
  ...
  const [mornight, setMornight] = useState(new Date().getUTCHours());

  useEffect(() => {
    setMornight(new Date().getUTCHours())
    setInterval(() => {
  ...
  return (
    <div className='flex flex-col'>
      <div className="flex flex-row justify-between items-center px-4 md:px-24 pt-20 sm:pt-12">
        <div className="flex flex-row gap-3">

          <div className="flex flex-row justify-center items-center gap-1 py-2">
            <LocationPin />
            <span className={`text-soft text-xs ${styles.location}`}>
              London, UK
            </span>
          </div>
          <div className="flex flex-row justify-center items-center gap-1 py-2">
            {mornight >= 6 && mornight < 18 ? <Sun /> : <Moon />}
            <span className={`text-soft text-xs ${styles.time}`}>
              {dateState || 'checking time...'}
            </span>
          </div>
        </div>
        <ThemeSwitcher />
      ...
      <div className="z-10 relative flex flex-col items-center justify-center max-w-xl mx-auto mt-[10%] gap-2 ">
        <>
          <Image className='hidden dark:block' src="/images/me-draw-dark.png" alt="illustration of Dom" width={48} height={48} />
          <Image className='block dark:hidden' src="/images/me-draw.png" alt="illustration of Dom" width={48} height={48} />
        </>
        <h1 className="text-xl text-sharp ">Hi, I am Dom</h1>
